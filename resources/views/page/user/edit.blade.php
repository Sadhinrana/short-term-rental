                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    <input type="text" name="name" class="form-control input-border @error('name') is-invalid @enderror" value="{{ $user->name }}" placeholder="Enter name">
                                    @error('name')
                                ...
                                <div class="form-group col-md-4">
                                    <input type="email" name="email" class="form-control input-border @error('email') is-invalid @enderror" value="{{ $user->email }}" placeholder="Enter email">
                                    @error('email')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="form-group col-md-4">
                                    <select class="form-control input-border @error('role') is-invalid @enderror" name="role">
                                        ...
                                        <option value="0" @if(!$user->role) selected @endif>User</option>
                                ...
                                <div class="form-group col-md-4">
                                    <select class="form-control select2 @error('city') is-invalid @enderror" multiple name="city[]">
                                        @foreach($masterProperty as $row)
                                            <option value="{{ $row->name }}" @if ($user->communities->contains('community', $row->name)) selected @endif>{{ $row->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('city')
                                    <span class="invalid-feedback" role="alert">
